...
function lobbyState() {
  return { users: Array.from(usersByUserId.values()).map(({ socketId, ...pub }) => pub) };
}

function broadcastLobby() { io.to("lobby").emit("LOBBY_STATE", lobbyState()); }
...
function emitYtState(session: SessionRecord) {
  io.to(`session:${session.sessionId}`).emit("YT_STATE", {
    sessionId: session.sessionId,
    videoId: session.ytVideoId,
    playing: session.ytPlaying,
    time: session.ytTime,
  });
}
...
// ── Start a session ───────────────────────────────────────────────────────────
function startSession(fromUser: UserRecord, toUser: UserRecord, activityType: ActivityType) {
  ...
  broadcastLobby();
  console.log("SESSION_STARTED", sessionId, activityType);
}
...
app.post("/slack/invite", async (req: Request, res: Response) => {
  const { slackUsername, activityType, fromDisplayName, fromUserId } = req.body as {
    slackUsername: string; activityType: ActivityType;
    fromDisplayName: string; fromUserId: string;
  };
  ...
  if (!slackUserId) { res.status(404).json({ ok: false, error: "Slack user not found" }); return; }

  const token = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
  slackInviteLinks.set(token, {
    activityType, fromDisplayName, fromUserId,
    expires: Date.now() + 10 * 60 * 1000,
  });

  const joinUrl   = `${CLIENT_URL}?slackInvite=${token}&from=${encodeURIComponent(fromDisplayName)}`;
  const gameLabel = activityType === "chess" ? "Chess ♟️" : activityType === "connect4" ? "Connect 4 🔴" : "YouTube 🎬";

  const blocks = [
    { type: "section", text: { type: "mrkdwn", text: `*${fromDisplayName}* is inviting you to *${gameLabel}* on Breakroom!` } },
    { type: "actions", elements: [{ type: "button", text: { type: "plain_text", text: "Join 🎮" }, style: "primary", url: joinUrl }] },
    { type: "context", elements: [{ type: "mrkdwn", text: "Invite expires in 10 minutes. You'll be dropped straight in!" }] },
  ];

  const ok = await sendSlackDM(slackUserId, `${fromDisplayName} invited you: ${joinUrl}`, blocks);
...
app.get("/slack/invite/:token", (req: Request, res: Response) => {
  ...
  if (!link) { res.status(404).json({ ok: false, error: "Invalid or expired invite" }); return; }
  if (Date.now() > link.expires) {
    slackInviteLinks.delete(req.params.token);
    res.status(410).json({ ok: false, error: "Invite expired" }); return;
  }
  res.json({ ok: true, activityType: link.activityType, fromDisplayName: link.fromDisplayName, fromUserId: link.fromUserId });
});
...
io.on("connection", (socket) => {
    ...
    if (payload.slackInviteToken) {
      ...
      if (link && Date.now() <= link.expires) {
        ...
        if (fromUser && fromUser.presence === "in_lobby") {
          ...
          socket.emit("SLACK_INVITE_UNAVAILABLE", { fromDisplayName: link.fromDisplayName });
        }
      } else {
  ...
  // ── YouTube events ────────────────────────────────────────────────────────
  // Load a new video (either user can pick)
  socket.on("YT_LOAD", (payload: { sessionId: string; videoId: string }) => {
    const userId = userIdBySocketId.get(socket.id);
    if (!userId) return;
    const session = sessionsById.get(payload.sessionId);
    if (!session || session.status !== "active" || session.activityType !== "youtube") return;
    if (!session.userIds.includes(userId)) return;

    session.ytVideoId = payload.videoId;
    session.ytPlaying = false;
    session.ytTime    = 0;
    sessionsById.set(session.sessionId, session);
    emitYtState(session);
  });

  // Sync play/pause/seek from one user to the other
  socket.on("YT_SYNC", (payload: { sessionId: string; playing: boolean; time: number }) => {
    const userId = userIdBySocketId.get(socket.id);
    if (!userId) return;
    const session = sessionsById.get(payload.sessionId);
    if (!session || session.status !== "active" || session.activityType !== "youtube") return;
    ...
    session.ytPlaying = payload.playing;
    session.ytTime    = payload.time;
    sessionsById.set(session.sessionId, session);

    // Broadcast only to the OTHER user in the session (not back to sender)
    const roomName   = `session:${session.sessionId}`;
    socket.to(roomName).emit("YT_STATE", {
      sessionId: session.sessionId,
      videoId:   session.ytVideoId,
      playing:   session.ytPlaying,
      time:      session.ytTime,
